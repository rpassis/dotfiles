...
class ReactDirManager {
    ...
    dispose() {
        new entryPointHandler_1.EntryPointHandler(entryPointHandler_1.ProcessType.Extension, new outputChannelLogger_1.OutputChannelLogger(vscode.window.createOutputChannel("React-Native"))).runFunction("extension.deleteTemporaryFolder", errorHelper_1.ErrorHelper.getInternalError(internalErrorCode_1.InternalErrorCode.RNTempFolderDeletionFailed, ReactDirManager.ReactDirPath), () => new fileSystem_1.FileSystem().removePathRecursivelySync(ReactDirManager.ReactDirPath));
    }
}
