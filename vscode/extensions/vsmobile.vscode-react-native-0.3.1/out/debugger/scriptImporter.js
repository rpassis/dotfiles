...
class ScriptImporter {
    constructor(packagerPort, sourcesStoragePath) {
    ...
    downloadAppScript(scriptUrlString) {
        ...
        return new request_1.Request().request(overriddenScriptUrlString, true).then(scriptBody => {
            ...
            let waitForSourceMapping = Q(null);
            if (sourceMappingUrl) {
    ...
    downloadDebuggerWorker(sourcesStoragePath) {
        return packager_1.Packager.isPackagerRunning(packager_1.Packager.getHostForPort(this.packagerPort))
            .then(running => {
            if (running) {
                let debuggerWorkerURL = `http://${packager_1.Packager.getHostForPort(this.packagerPort)}/${ScriptImporter.DEBUGGER_WORKER_FILENAME}`;
                ...
                return new request_1.Request().request(debuggerWorkerURL, true).then((body) => {
