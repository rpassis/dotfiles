...
function login(username, password) {
    return getPackage()
        .then((xdl) => xdl.User.loginAsync({ username: username, password: password }));
}
exports.login = login;
function mapVersion(reactNativeVersion) {
    return getPackage()
        .then((xdl) => xdl.Versions.facebookReactNativeVersionToExponentVersionAsync(reactNativeVersion));
}
...
function startExponentServer(projectRoot) {
    return getPackage()
        .then((xdl) => xdl.Project.startExponentServerAsync(projectRoot));
}
