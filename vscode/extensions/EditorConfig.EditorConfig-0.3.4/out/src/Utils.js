...
function fromEditorConfig(config, defaults) {
    const resolved = {
        tabSize: (config.indent_style === 'tab'
            ? lodash_1.get(config, 'tab_width', config.indent_size)
            : lodash_1.get(config, 'indent_size', config.tab_width))
    };
    if (lodash_1.get(resolved, 'tabSize') === 'tab') {
    ...
    return {
        ...
        tabSize: lodash_1.get(resolved, 'tabSize', defaults.tabSize)
    };
}
