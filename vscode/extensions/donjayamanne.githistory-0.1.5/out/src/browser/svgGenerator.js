...
(function () {
    ...
    */
    ...
    window.GITHISTORY.generateSVG = function () {
        ...
        let items = JSON.parse(document.querySelectorAll('div.json.entries')[0].innerHTML, dateReviver);
        if (logView !== null && items.length !== 0) {
            svg = logView.children[0];
            content = logView.children[1];
    ...
    ;
    function drawGitGraph(startAt, logEntryHeight = 60.8, entries) {
        // Draw the graph
        ...
        for (let i = startAt; i < content.children.length; ++i) {
            ...
            let left = element ? Math.max(minLeft, element.branchesOnLeft) : minLeft;
